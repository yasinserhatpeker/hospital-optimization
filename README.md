...
*   **Time Complexity:** Geliştirdiğimiz ameliyathane planlama algoritması, doğası gereği NP-Hard sınıfında yer alan ve standart kurgularda üstel (exponential) zaman karmaşıklığına sahip olan bir problemdir. Ancak sistemin işlemci yükünü optimize etmek ve olası darboğazları önlemek için arama uzayını daraltan stratejik bir yaklaşım izledik. İlk olarak, hastaları aciliyet ve ameliyat sürelerine göre sezgisel (heuristic) olarak sıralayarak algoritmaya O(n log n) maliyetli bir ön hazırlık yaptırdık. Bu sayede motorun rastgele denemeler yapmak yerine doğrudan en zorlu vakalara odaklanmasını sağladık. İkinci aşamada, kısıt motorumuz aracılığıyla mantıksız atama ihtimallerini henüz ağacın kökündeyken budadık (pruning), böylece milyonlarca gereksiz kombinasyonun işlemciyi meşgul etmesini engelledik. Son olarak, geçerli çözüm havuzunu sabit bir üst limit (C=20) ile sınırlandırarak algoritmanın mükemmeliyetçi sonsuz döngülere girmesinin önüne geçtik. Tüm bu mimari optimizasyonlar sonucunda, teorikte sistemleri çökertme potansiyeli olan bu problemi, operasyonel çalışma zamanında O(n log n + C x n) civarında, son derece ölçeklenebilir ve lineer (doğrusal) bir performansa yaklaştırmayı başardık.

*   **Space Complexity:** Geliştirdiğimiz sistemin bellek (RAM) tüketimini minimize etmek ve yüksek ölçeklenebilirlik sağlamak amacıyla, donanımı yoran maliyetli veritabanı (I/O) okuma-yazma işlemlerini tamamen devreden çıkaran "In-Memory" (bellek içi) bir veri işleme mimarisi tasarladık. Hastanedeki tüm kaynakların (odalar, cerrahlar ve ekipler) doluluk durumlarını karmaşık objeler yerine, sadece mesai saatlerini temsil eden 20 slotluk hafif Boolean (True/False) matrisleriyle modelledik. Bunun yanı sıra, kullandığımız Backtracking algoritmasının özyineleme derinliği (recursion depth), sistemdeki toplam hasta sayısı (n) ile sınırlı kalacak şekilde kurgulandığı için bellek taşması (StackOverflow) riskini güvenli bir şekilde ortadan kaldırdık. Tüm bu stratejik kararlar neticesinde, sistemin toplam alan karmaşıklığını O(n + (R + S + A) x T) (R=Oda sayısı), (S=Cerrah sayısı) (A=Takım sayısı) (T=slot sayısı) gibi donanım kaynaklarını neredeyse hiç yormayan, optimum bir düzeye çektik. Bu hafif ve verimli yapı, projemizin aynı anda birden fazla hastanenin operasyon planlamasını sunucu darboğazı yaratmadan yönetebilecek kurumsal (enterprise) bir kapasiteye sahip olduğunu kanıtlamaktadır.
